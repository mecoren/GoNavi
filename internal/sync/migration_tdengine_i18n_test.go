...
func TestTDengineMigrationUsesLocalizedBackendTextSourceGuard(t *testing.T) {
	sourceBytes, err := os.ReadFile("migration_tdengine.go")
	if err != nil {
		t.Fatalf("read migration_tdengine.go: %v", err)
	}
	...
	for _, legacy := range legacyMessages {
		if strings.Contains(source, legacy) {
			t.Fatalf("migration_tdengine.go still contains legacy raw user-visible message %q", legacy)
		}
	}
	...
	for _, key := range requiredKeys {
		if !strings.Contains(source, key) {
			t.Fatalf("migration_tdengine.go should reference localized key %q", key)
		}
	}
}
